...
i386_detect_memory(void)
{
	...
	cprintf("Physical memory: %uK available, base = %uK, extended = %uK, PG_Size = %uB, Npages = %u\n",
		totalmem, basemem, totalmem - basemem, PGSIZE,npages);
    //cprintf("Testing end: %u\n", (uint32_t)end);
    //extern char end[];
    //static char *nextfree;
    //nextfree = ROUNDUP((char *) end, PGSIZE);
    //cprintf("Movement Test: %u,Kernbase: %u Testing free size = %u\n",  (uint32_t)nextfree - KERNBASE,KERNBASE,(uint32_t)nextfree);
}
...
boot_alloc(uint32_t n)
{
    ...
    if( n > 0 ){
        ...
        //panic("We have an error at line 116");
        result = nextfree;
        return result;
    }
    else if ( n == 0 ){
...
//
// From UTOP to ULIM, the user is allowed to :wq
// :read but not write.
...
mem_init(void)
{
    ...
    //panic("We get past boot alloc 1 ");
	...
	// Your code goes here:
    //istruct PageInfo pages[npages];
    pages = (struct PageInfo*)boot_alloc( npages * (sizeof(struct PageInfo*)) );
    memset(pages, '\0', npages);

    //panic("We get past boot alloc");
	//////////////////////////////////////////////////////////////////////
...
//    boot_map_region(kern_pgdir, (uintptr_t)envs, env_size, PADDR(envs), PTE_W | PTE_P);
    //////////////////////////////////////////////////////////////////////
	...
	// Your code goes here:
   // pde_t pde = PDX(KERNBASE);
    //cprintf("Test Meme again: %d\n", pde);
    boot_map_region(kern_pgdir, KSTACKTOP-KSTKSIZE, KSTKSIZE, PADDR(bootstack), PTE_W | PTE_P);
   // boot_map_region(kern_pgdir, KSTACKTOP-PTSIZE, KSTACKTOP-KSTKSIZE, PADDR(bootstack), PTE_U | PTE_P);
   //////////////////////////////////////////////////////////////////////
    ...
    //int range = (uint32_t)(1) - (KERNBASE+1);
    //uint32_t range2 = (2^32) - KERNBASE;
    //cprintf("1: %u\n", range);
    //cprintf("2: %u\n", -KERNBASE);
...
page_init(void)
{
	...
	// free pages!
	size_t i;
    size_t markMP = PGNUM(MPENTRY_PADDR); // Is 7 according to print statement
    //cprintf("Markmp: %d\n", markMP);
    //panic("Testing a value");
    /* Next free calculation*/
...
{
	// Fill this function in
    pde_t directory = PDX(va); // Okay, we have the page directory index.
    pde_t pde = pgdir[PDX(va)];
    pte_t table = PTX(va); // Table index, I'm not sure if we'll need this.
    //pte_t offset = PGOFF(va); // We can build the whole linear address now. Possible use?
    pte_t *holder; //This should hold the "va" page table BEFORE we add our new page.
    //Now,
    if(pde & PTE_P){ // Page exists, note that the & operator is a bitwise operator
        ...
        // Okay, we have found our page, so essentially should do an offset calculation and find the pointer position.
    }

    else if(!(pde & PTE_P)){ // Okay, no page found.
        if(create){
            ...
            holder = (pte_t*)KADDR(page2pa(pNew)); // I think PTE_ADDR works here too? We'll check later.
        }
        else
            return NULL;
    }
...
{
	// Fill this function in
    // "Map", I think we need to use page2kkva here (found in pmap.h)
    // VA = Virtual Address
    // PA = Physical Address
    // Perm, My guess is we would pass in a perm (like PTE_W) in, so let's just use the argument for now.
    // My guess is we're "mimicing" the virtual address map to the physical.
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
    ...
    if(pp == NULL || !(*pp & PTE_P))
        return NULL;
    //physaddr_t pa = page2pa(pp); // Not sure what this is for, but the hint mentions it.
...
user_mem_check(struct Env *env, const void *va, size_t len, int perm)
{
    ...
    for(; start_va < end_va; start_va += PGSIZE){
        ...
        if( (pp == NULL) || (start_va >= ULIM) || !(*pp & PTE_P) || ((*pp & perm) != perm)){
            ...
            //user_mem_check_addr = (start_va < (uint32_t)va ? (uint32_t)va : start_va);//(uintptr_t)start_va;
            return -E_FAULT;
        }
