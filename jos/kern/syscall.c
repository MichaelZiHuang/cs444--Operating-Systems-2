...
sys_cputs(const char *s, size_t len)
{
   ...
   /* for(size_t i = sizeof(s); i < sizeof(s+len); i+= 4){
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	// LAB 4: Your code here.
	int r;
    struct Env *e; // Prob need to get this using the envid.
    pte_t *pte;
    struct PageInfo *p;
    // It mentions a lot of pages, we need to send a page and make sure we actually have that page.
    // We can probably find a lot of the structure by looking at the errors (just like how we figured out the other ones)

    // First, we need to get the env. It mentioed something about envid2env being dangerous?
    ...
    if(srcva < (void *)UTOP){
        ...
        // We passed ALL the tests, so it's time send the page.
        ...
        e->env_ipc_perm = 0;
    //e->env_ipc_value = value;
   // r = page_insert(e->env_pgdir, p, srcva, perm);
    e->env_ipc_recving = 0;
